...
class Channel_1(nn.Module):
    ...
    def forward(self, x1,x2,x3) :
        max_result1=self.maxpool(x1)
        avg_result1=self.avgpool(x1)
        
        Resultant1=max_result1+avg_result1
        
        max_result2=self.maxpool(x2)
        avg_result2=self.maxpool(x2)
        Resultant2=max_result2+avg_result2
        
        max_result3=self.maxpool(x3)
        avg_result3=self.maxpool(x3)
        Resultant3=max_result3+avg_result3
        
        Resultant= Resultant1+Resultant2+Resultant3
        
        Resultant=self.se(Resultant)
...
class Attention_Block(nn.Module):
    ...
    def __init__(self,channel,reduction=16):
        super().__init__()
        self.A1=Channel_1(channel=channel,reduction=reduction)
...
class m_unet(nn.Module):
    ...
    def __init__(self, n_class=1):
        ...
        self.att_1=Attention_Block(16)
        self.att_2=Attention_Block(32)
        self.att_3=Attention_Block(64)
        self.att_4=Attention_Block(128)
        self.att_5=Attention_Block(256)
        self.att_6=Attention_Block(512)
        self.att_7=Attention_Block(1024)
...
#
        ...
        zero_1=torch.zeros([batch,512,10,10])
        ...
        zero_2_1=torch.zeros([batch,768,20,20])
        zero_2_2=torch.zeros([batch,512,20,20])
        ...
        zero_3_1=torch.zeros([batch,384,40,40])
        zero_3_2=torch.zeros([batch,256,40,40])
        ...
        zero_4_1=torch.zeros([batch,192,80,80])
        zero_4_2=torch.zeros([batch,128,80,80])
        ...
        zero_5_1=torch.zeros([batch,96,160,160])
        zero_5_2=torch.zeros([batch,64,160,160])
        zero_5_3=torch.zeros([batch,112,160,160])
        ...
        zero_6_1=torch.zeros([batch,32,320,320])
        zero_6_2=torch.zeros([batch,48,320,320])
        ...
        zero_7_1=torch.zeros([batch,16,640,640])        
